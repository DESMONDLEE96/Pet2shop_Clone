...
class ModelLogisticShipping extends Model {
	public function addShippingCourier($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "shipping_courier SET shipping_courier_code = '" . $this->db->escape($data['shipping_courier_code']) . "', shipping_courier_name = '" . $this->db->escape($data['shipping_courier_name']) . "', store_id = '". (int)$this->seller->getStoreId() ."'");
		
		return $this->db->getLastId();
	}

	public function editShippingCourier($shippingCourierId, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "shipping_courier SET shipping_courier_code = '" . $this->db->escape($data['shipping_courier_code']) . "', shipping_courier_name = '" . $this->db->escape($data['shipping_courier_name']) . "', store_id = '". (int)$this->seller->getStoreId() ."' WHERE shipping_courier_id = '" . $shippingCourierId . "'");
	}
	...
	public function getShippingCouriers($data = array()) {
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "shipping_courier WHERE store_id = '". (int)$this->seller->getStoreId() ."'";
	...
	public function addOrEditShippingLocation($data = array(), $shipping_location_id = 0) {
		...
		if ($shipping_location_id == 0) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "shipping_location SET shipping_courier_id = '" . $this->db->escape($data['shipping_courier_id']) . "', country_id = " . $country_id . ", geo_zone_id = " . $geo_zone_id . ", weight_setting = '" . $this->db->escape(html_entity_decode($data['weight_setting'])) ."', store_id = '". (int)$this->seller->getStoreId() ."', status='" .(int)$data['status']. "'");
		} else {
			$this->db->query("UPDATE " . DB_PREFIX . "shipping_location SET shipping_courier_id = '" . $this->db->escape($data['shipping_courier_id']) . "', country_id = " . $country_id . ", geo_zone_id = " . $geo_zone_id . ", weight_setting = '" . $this->db->escape(html_entity_decode($data['weight_setting'])) ."', store_id = '". (int)$this->seller->getStoreId() ."', status='" .(int)$data['status']. "' WHERE id = '" . (int)$shipping_location_id . "'");
		}
	...
	public function getShippingLocations($data = array()) {
		$query = $this->db->query("SELECT a.*, b.* FROM ". DB_PREFIX . "shipping_location a, ". DB_PREFIX ."shipping_courier b WHERE a.store_id = '". (int)$this->seller->getStoreId() ."' AND a.shipping_courier_id = b.shipping_courier_id");
		$ship_loc = $query->rows;
		$geo_zone_ids = array();
		$country_ids = array();
