...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		$variation = $this->db->escape(json_encode($data['variation']));

		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', variation = '". $variation ."', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (isset($data['points']) ? (int)$data['points'] : 0) . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', store_id = '". (int)$this->seller->getStoreId() ."', date_added = NOW(), date_modified = NOW()");
	...
	public function getProductOrderedNumber($product_id) {
		$query = $this->db->query("SELECT SUM(quantity) AS total_order FROM  " . DB_PREFIX . "order_product WHERE product_id = '". (int)$product_id ."'");
	...
	public function getProducts($data = array()) {
		...
		//for seller only get their own products
		$sql .= " AND store_id = '". (int)$this->seller->getStoreId() ."' GROUP BY p.product_id";
	...
	public function getTotalProducts($data = array()) {
		...
		//for seller only get their own products
		$sql .= " AND store_id = '". (int)$this->seller->getStoreId() ."'";
	...
	public function updateProductCoverImage($product_id, $image_url) {
