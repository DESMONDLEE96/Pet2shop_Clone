...
class ControllerCommonLogin extends Controller {
	...
	public function index() {
		...
		if ($this->seller->isLogged() && isset($this->request->get['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
			$this->response->redirect($this->url->link('store/store', 'user_token=' . $this->session->data['user_token'], true));
		}
	...
	protected function validate() {
		if (!isset($this->request->post['email']) || !isset($this->request->post['password']) || !$this->seller->login($this->request->post['email'], html_entity_decode($this->request->post['password'], ENT_QUOTES, 'UTF-8'))) {
