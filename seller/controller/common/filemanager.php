...
class ControllerCommonFileManager extends Controller {
	...
	public function uploadProductImages() {
		...
		if (isset($this->request->get['product_id'])) {
			// $directory = rtrim(DIR_IMAGE . 'catalog/product/' . $this->request->get['product_id'], '/'); //should use this 1
			$directory = rtrim(DIR_IMAGE . 'catalog/product/' . $this->request->get['product_id'], '/');
		} else {
			$json['error'] = "No product ID";
		}

		// Check the directory exist, if not den create directory
		if (!is_dir($directory)) {
			mkdir($directory . '/' . $this->request->get['product_id'], 0777, true);
			chmod($directory . '/' . $this->request->get['product_id'], 0777);
		}

		//check directory again after created
		if (!$json && is_dir($directory)) {
			...
			if (!empty($this->request->files['file']['name']) && is_array($this->request->files['file']['name'])) {
				foreach (array_keys($this->request->files['file']['name']) as $key) {
					//$fileType = explode('.', $this->request->files['file']['name'][$key]);
			...
			$hasCoverImg = !empty($product['image']) ? true : false;
			...
			foreach ($files as $file) {
				if (is_file($file['tmp_name'])) {
					...
					if (!in_array($file['type'], $allowed2)) {
						$json['error'] = $this->language->get('error_filetype_picture');
					}
					...
					//check has cover image or not
					if ($hasCoverImg == false && explode('.', $file['name'])[0] == 'img_1') { //img 1 is cover image.
						$hasCoverImg = true;
					}
				} else {
					$json['error'] = $this->language->get('error_upload');
				...
				//if no cover image
				if ($hasCoverImg == false) {
					$json['error'] = "Please Upload An Cover Image";
				}
				...
				if (!$json) {
					...
					foreach($allowed as $a) {
						if (file_exists($directory . '/' . $tmpName[0].'.'.$a)) {
							unlink($directory . '/' . $tmpName[0].'.'.$a);
					...
					//if is img_1 update as new cover image
					if ($tmpName[0] == 'img_1') {
						$directory2 = rtrim(HTTPS_CATALOG . 'image/catalog/product/' . $this->request->get['product_id'], '/'); //on live need to change this
						$this->model_catalog_product->updateProductCoverImage($this->request->get['product_id'], $directory2 . '/' . $filename);
					}
				}
			}
		}
	...
	public function deleteProductImage() {
		...
		if (!$json) {
			$file = rtrim(DIR_IMAGE . 'catalog/product/' . $this->request->get['product_id'] . '/' .$this->request->post['filename'], '/');
