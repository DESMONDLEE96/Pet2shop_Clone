...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $k => $result) {
			...
			$var = json_decode($result['variation'], true);
			if (!empty($var)) { // have variation take variation total amount
				$var = json_decode($result['variation'], true);
				$total = 0;
				$varL = $var['variationList'];
				if (!empty($var['variation2Name']) && !empty($var['variation2Options'])) { //have 2 variation
					foreach($varL as $list) {
						foreach($list as $l) {
							$total+= $l['stock'];
						}
					}
				} else { //only 1 variation
					foreach($varL as $list) {
						$total+= $list['stock'];
					}
				}
	...
	protected function getForm() {
		...
		if(!empty($product_info)) {
			$total_ordered = $this->model_catalog_product->getProductOrderedNumber($this->request->get['product_id']);

			$data['total_order'] = $total_ordered;
		}
		...
		$data['product_images'] = isset($this->request->get['product_id']) ? $this->getAllProductImages($this->request->get['product_id']) : "";
	...
	protected function validateForm($data = array()) {
		...
		$data = !empty($data) ? $data : $this->request->post;

		foreach ($data['product_description'] as $language_id => $value) {
			if ((utf8_strlen($value['name']) < 1) || (utf8_strlen($value['name']) > 255)) {
				array_push($tmp, $this->language->get('error_name'));
			}

			if ((utf8_strlen($value['meta_title']) < 1) || (utf8_strlen($value['meta_title']) > 255)) {
				array_push($tmp, $this->language->get('error_meta_title'));
			}
		}
	...
	public function editProduct() {
		...
		if ($this->validateForm($param)) {
			...
			$variation = !empty($variation['variation1Name']) ? $variation : NULL;
	...
	public function addProduct() {
		...
		if ($this->validateForm($param)) {
			...
			$json['redirect_url'] = $this->url->link('catalog/product/edit', 'product_id=' . $product_id, true);
