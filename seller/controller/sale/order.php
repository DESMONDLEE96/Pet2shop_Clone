...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				'order_id'      => $result['sub_order_id'],
				'customer'      => $result['customer'],
				'order_status'  => $result['order_status'] ? $result['order_status'] : $this->language->get('text_missing'),
				'total'         => $this->currency->format($result['total'], $result['currency_code'], $result['currency_value']),
				'date_added'    => date($this->language->get('date_format_short'), strtotime($result['created_datetime'])),
				'view'          => $this->url->link('sale/order/info', 'user_token=' . $this->session->data['user_token'] . '&sub_order_id=' . $result['sub_order_id'] . $url, true),
				'edit'          => $this->url->link('sale/order/edit', 'user_token=' . $this->session->data['user_token'] . '&sub_order_id=' . $result['sub_order_id'] . $url, true)
			);
		}
	...
	public function info() {
		...
		if (isset($this->request->get['sub_order_id'])) {
			$sub_order_id = $this->request->get['sub_order_id'];
		} else {
			$sub_order_id = 0;
		}

		$order_info = $this->model_sale_order->getSubOrder($sub_order_id);
		...
		if ($order_info) {
			...
			$data['text_order'] = sprintf($this->language->get('text_order'), $this->request->get['sub_order_id']);

			$url = '';

			if (isset($this->request->get['filter_sub_order_id'])) {
				$url .= '&filter_sub_order_id=' . $this->request->get['filter_sub_order_id'];
			}
			...
			$data['sub_order_id'] = $this->request->get['sub_order_id'];

			$data['date_added'] = date('d/m/Y g:iA', strtotime($order_info['date_added']));
			...
			$products = $this->model_sale_order->getSubOrderProducts($this->request->get['sub_order_id']);
			...
			$data['courier'] = $order_info['courier'];
			$data['comment'] = $order_info['comment'];
			$data['product_total'] = $this->currency->format($order_info['product_total'], $order_info['currency_code'], $order_info['currency_value']);
			$data['shipping_total'] = $this->currency->format($order_info['shipping_total'], $order_info['currency_code'], $order_info['currency_value']);
	...
	public function itemShipped() {
		...
		if (isset($this->request->get['sub_order_id'])) {
			...
			$json['error'] = "Invalid sub order ID";
		}

		if (isset($this->request->post['tracking_no']) && !empty($this->request->post['tracking_no'])) {
			$tracking_no = $this->request->post['tracking_no'];
		} else {
			$json['error'] = "Invalid tracking no";
		}

		if (!isset($json['error'])) {
			$this->load->model('sale/order');
			$this->model_sale_order->itemShipped($sub_order_id, $tracking_no);
		}

		$json['success'] = "Successfully Updated Order to Shipped Out";
